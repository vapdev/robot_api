...
def preenche_suporte(ws, planilha, sols_filhas, db, sols_ocupadas, sol_pai):
    horas_total = 0 
    count=0
    ...
    for i, nr_solic in enumerate(sols_filhas):
        ...
        start = acha_start('suporte', ws)
        
        if (tp_solicitacao in [2,3,10,12] and nr_solicitacao not in sols_ocupadas and "deslocamento" not in ds_assunto.lower()) or "mestra" in ds_assunto.lower() :
